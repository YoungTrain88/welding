...
try:
    from ultralytics.utils.ops import make_divisible
    # 正常导入我们需要的所有模块
    from ultralytics.nn.modules import (Conv, C2f, Bottleneck, SPPF, C3, DWConv, 
                                        SCDown, C3k2, PSA, ABlock, A2C2f)
    
    # 将官方的 Concat 导入，但给它起一个别的名字，比如 OfficialConcat
    from ultralytics.nn.modules import Concat as OfficialConcat
    ...
    raise ImportError(f"无法从 'ultralytics' 导入必要的模块。请确保已正确安装。错误: {e}")
# ==================================================================
...
def parse_custom_model(d, ch, verbose=True):
    ...
    nc = d.get('nc')
    if not isinstance(nc, int): raise TypeError(f"YAML配置错误: 'nc' 必须在YAML文件的顶部定义为一个整数，但现在的值是 {nc}。")
    
    gd = d.get('depth_multiple', 1.0)
    gw = d.get('width_multiple', 1.0)
    
    available_modules = [Conv, C2f, C3, A2C2f, C3k2, Bottleneck, PSA]
    if C2fCIB: available_modules.append(C2fCIB)
    if C2PSA: available_modules.append(C2PSA)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):
        ...
        # 解析器现在变得更简单，因为它看到的 Concat 永远是我们自己的版本
        try:
            m_class = globals().get(m_str) or getattr(torch.nn, m_str)
        except (AttributeError, TypeError):
            try:
                m_class = getattr(__import__('ultralytics.nn.modules', fromlist=[m_str]), m_str)
            except (ImportError, AttributeError):
                raise NameError(f"无法解析模块 '{m_str}'。请检查YAML中的拼写或升级您的 'ultralytics' 包。")
        ...
        if m in available_modules:
            ...
            if c2 != nc: c2 = make_divisible(c2 * gw, 8)
            args = [c1, c2, *args[1:]]
            if m in [C2f, A2C2f, C3, C3k2] or (C2fCIB and m is C2fCIB) or (C2PSA and m is C2PSA):
                args.insert(2, n)
                n = 1
        elif m in [ CARC]:
            c1 = ch[f]
            ...
            c1, c2 = ch[f], args[0]; c2 = make_divisible(c2 * gw, 8) if c2 != nc else c2; args = [c1, c2, *args[1:]]
        elif m is Concat: # 现在 Concat 就是我们的 CustomConcat
            c2 = sum(ch[x] for x in f)
        elif m in [nn.Upsample, nn.MaxPool2d]:
            c2 = ch[f]
        elif m is PSA:
             c1 = c2 = ch[f]; args = [c1, *args[1:]]
        elif m is RegressionHead:
            c1 = sum(ch[x] for x in f); c2 = 1; args = [c1]
        elif m in [AFGCAttention, PSA]:
            c1 = c2 = ch[f]; args = [c1]
        else:
            c2 = ch[f]
...
class CustomConcat(nn.Module):
    ...
    def forward(self, x):
        ...
        max_w = max(img.shape[3] for img in x)
...
# ==================== 偷天换日！ ====================
# 在本文件的作用域内，我们声明 Concat 这个名字现在就指向我们自己的 CustomConcat 类
Concat = CustomConcat
# ================================================
class RegressionHead(nn.Module):
    # ... RegressionHead 的代码保持不变 ...
    ...
    def forward(self, x, return_features=False):
        if isinstance(x, list):
            target_size = x[0].shape[2:]
            x = [ F.interpolate(img, size=target_size, mode='bilinear', align_corners=False) if img.shape[2:] != target_size else img for img in x]
            x = torch.cat(x, 1)
