...
def run_training_session(model, model_name, train_loader, val_loader, save_dir):
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"结果 -> 训练损失: {avg_train_loss:.4f} | 验证损失: {avg_val_loss:.4f} | 验证MAE: {val_mae:.4f} | 验证R2: {val_r2:.4f}")
...
if __name__ == '__main__':
    ...
    for model_name in models_to_train:
        ...
        late_fusion_model = LateFusionMultiModalNet(
            image_regression_model=image_regression_model,
            num_tabular_features=num_tabular_features
        )
        
        # (可选) 打印模型结构
        print("\n--- 模型结构摘要 ---")
        summary(late_fusion_model, 
