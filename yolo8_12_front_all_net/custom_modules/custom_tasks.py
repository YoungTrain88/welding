...
class AFGCAttention(nn.Module):
    ...
    def forward(self, x):
        y = self.avg_pool(x); y = self.conv(y.squeeze(-1).transpose(-1, -2)).transpose(-1, -2).unsqueeze(-1); y = self.sigmoid(y); return x * y.expand_as(x)

class ARCBlock(nn.Module):
    def __init__(self, c1, c2, shortcut=True):
        super().__init__()
        self.cv1 = Conv(c1, c2, 3, 1); self.cv2 = Conv(c2, c2, 3, 1); self.add = shortcut and c1 == c2
    def forward(self, x):
...
class CARC(nn.Module):
    def __init__(self, c1, c2, n=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e); self.cv1 = Conv(c1, c_, 1, 1); self.cv2 = Conv(c1, c_, 1, 1); self.cv3 = Conv(2 * c_, c2, 1); self.m = nn.Sequential(*(ARCBlock(c_, c_) for _ in range(n)))
    def forward(self, x):
...
def parse_custom_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):
        ...
        if isinstance(m, str):
            if m.startswith('nn.'):
                module_name = m[3:]
                try: m_class = getattr(nn, module_name)
                except AttributeError: raise AttributeError(f"模块 '{module_name}' 在 torch.nn 中未找到。")
            else:
                module_name = m
                # ==================== 关键修改点 1 ====================
                ...
                if m_class is None:
                    try: m_class = getattr(__import__('ultralytics.nn.modules', fromlist=[module_name]), module_name)
                    except (ImportError, AttributeError): pass
        else:
            m_class = m

        if m_class is None: raise NameError(f"无法解析模块 '{m}'。")
        m = m_class
