...
class RegressionHead(nn.Module):
    ...
    def __init__(self, c1: int, k: int = 1, s: int = 1, p: int | None = None, g: int = 1):
        super().__init__()
        c_ = 1280
...
class AFGCAttention(nn.Module):
    """一个修正后可运行的自适应细粒度通道注意力模块。 它接收一个输入张量，返回一个经过通道注意力加权后的张量，尺寸不变。.
    """
...
class CARC(nn.Module):
    """Context Aggregation and Refinement Block for CARC-Net 一个修正后可运行的CARC模块。.
    """
