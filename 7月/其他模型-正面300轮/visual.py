...
def get_target_layer(model, model_name):
    ...
    elif "densenet" in model_name or "vgg" in model_name or "mobilenet" in model_name:
...
def attention_rollout(model, image_tensor, image_pil, model_name):
    ...
    attn_mat = [a.numpy() for a in attn_weights]
    attn_mat = [np.mean(a, axis=1) for a in attn_mat if a.ndim >= 3]
    if not attn_mat or attn_mat[0].ndim < 2:
        raise RuntimeError("注意力权重shape异常，无法可视化。")
