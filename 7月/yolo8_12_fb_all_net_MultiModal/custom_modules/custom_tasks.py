...
try:
    # 正常导入我们需要的所有模块
    from ultralytics.nn.modules import C3, PSA, SPPF, A2C2f, ABlock, Bottleneck, C2f, C3k2, Conv, DWConv, SCDown
...
def parse_custom_model(d, ch, verbose=True):
    ...
    nc = d.get("nc")
    if not isinstance(nc, int):
        raise TypeError(f"YAML配置错误: 'nc' 必须在YAML文件的顶部定义为一个整数，但现在的值是 {nc}。")
    ...
    if C2fCIB:
        available_modules.append(C2fCIB)
    if C2PSA:
        available_modules.append(C2PSA)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):
        ...
        m_.i, m_.f, m_.type, m_.np = i, f, t, np
        if verbose:
            print(f"{i:>3}{f!s:>18}{n:>3}{np:10.0f}  {t:<40} {args!s:<30}")
...
class CustomConcat(nn.Module):
    """一个真正智能的拼接模块，它会自动找到输入中的最大空间尺寸， 并将所有较小的输入上采样到该尺寸，然后再进行拼接。.
    """
...
class RegressionModel(nn.Module):
    ...
    def initialize_biases(self, cf=None):
        ...
        for m in self.model.modules():
            if isinstance(m, nn.Conv2d) and hasattr(m, "bn") and isinstance(m.bn, nn.BatchNorm2d):
                with torch.no_grad():
                    m.bn.bias.zero_()
