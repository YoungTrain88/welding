...
class MultiModalModel(nn.Module):
    ...
    """
    def __init__(self, yaml_path, num_tabular_features, 
                 image_output_dim=8, 
                 tabular_output_dim=4,
                 hidden_dim=64, 
                 dropout_rate=0.5):
        super().__init__()
        ...
        self.image_feature_compressor = nn.Sequential(
            nn.Linear(original_image_feature_dim, image_output_dim),
            nn.ReLU()
        )
...
def train_one_yaml(yaml_path):
    ...
    # ##################################################################
    train_dataset = RegressionDataset(csv_path=TRAIN_CSV_PATH, project_root=PROJECT_ROOT, project_name=PROJECT_NAME, transform=train_transform)
    val_dataset = RegressionDataset(csv_path=VAL_CSV_PATH, project_root=PROJECT_ROOT, project_name=PROJECT_NAME, transform=val_transform)
